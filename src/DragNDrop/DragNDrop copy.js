  ...
  const handlersRef = useRef({
    onMouseDown: (ee) => {
      if(dragElRef.current !=null&&insertRef.current!=null) return
        ...
        console.log('mouseDown',dragElRef.current,insertRef.current)
    ...
    onLongPress: (ee) => {
      ...
      console.log(dragElRef.current)
      childrenRef.current[dragElRef.current].current.classList.add(
        classes.elevation
      );
      dragRef.current = true;
      /* rerenderChildren({ style: {} }); */
      console.log(ee.clientY)
      firstDragRef.current = ee.clientY;
      lastMouseRef.current = ee.clientY
      ...
      onTranslate(
        ...
        0,
        ee.clientY -
          dimensionsRef.current.offsetEltop -
    ...
    onMouseLeave: (i) => {},
    onMouseMove: (ee) => {
      edgeDetector(ee.clientX, ee.clientY);
      const draggable = childrenRef.current[dragElRef.current].current;
    ...
    onMouseUp: (ee) => {
      ...
      if (dragRef.current) {
        ...
        rerenderChildren({ onClick: (e) => {e.stopPropagation()} });
    ...
    onTransitionEnd: () => {
      ...
      lastTransitionRef.current = lastTransitionRef.current + 1;console.log(totalTransitionRef.current, lastTransitionRef.current)
    ...
    if (insertRef.current > dragElRef.current + 1) {
      ...
      if (insertRef.current == 0) {
        insertTranslate =
          ...
          dimGetter("top", dragElRef.current) +16;
      } else {
        insertTranslate = -(
          dimGetter("top", dragElRef.current) -
          dimGetter("bottom", insertRef.current - 1) -
